...
MO.FEaiChartCustomerSphereScene_onProcessReady = function FEaiChartCustomerSphereScene_onProcessReady(){
 ...
 //  image.addLoadListener(o, o.onImageLoad);
   // 创建图片
   var image  = imageConsole.load('{eai.resource}/world/color.jpg');
   image._ready = true;
   image.testReady();
   context2d.drawImage(image,0,0,width,hight);
   //context2d.drawImage(image,100,100,width,hight);
   var ctx = context2d._handle;
    ctx.lineCap = 'round';
    ctx.beginPath();
   var countries = o._worldResource.data().countries(),
    count = countries.count(),
    country,boundaries,boundary,positions,items,panX,panY,x,y,scale;
   for(var n = 0 ;  n < count ; n++){
    country = countries.at(n);
    ...
    for(var i = 0 ; i < boundaries._count ; i++){
     ...
     ctx.moveTo(position[0]* scalex + panX ,(-position[1])*scaley + panY);
      for(var j=0; j < positionCount; j++){
          x = position [0+j*2] * scalex + panX;
          y = (-position[1+j*2]) * scaley + panY;
          ctx.lineTo(x,y);
        }
    }
   }
